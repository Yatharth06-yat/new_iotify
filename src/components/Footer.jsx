...
import { Mail, MapPin, Linkedin, Twitter, Github, Youtube } from "lucide-react";
          ...
          {Object.entries(FOOTER_LINKS).map(([title, links], colIdx) => (
            ...
            >
              ...
              <ul className="flex flex-col gap-2.5">
                {links.map(({ label, page, externalUrl }) => (
                  <li key={label}>
                    {externalUrl ? (
                      <a
                        href={externalUrl}
                        ...
                        className="text-gray-600 text-sm hover:text-accent transition-colors duration-200 text-left font-medium focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent rounded inline-flex items-center gap-1.5"
                      >
                        {label}
                      </a>
                    ) : (
